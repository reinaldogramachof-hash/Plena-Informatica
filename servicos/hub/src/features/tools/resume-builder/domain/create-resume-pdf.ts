...
export function buildResumeDocument(
  ...
  const educationSection: ResumeDocumentSection = {
    ...
    items: resume.education.map((education) => {
      const endLabel = education.current
        ? 'Em curso'
        : (education.endYear ?? '')
      return `${education.course} - ${education.institution}\n${formatPeriod(
...
async function renderClassicAts(document: ResumeDocument): Promise<Uint8Array> {
  ...
  const { regular: regularFont, bold: boldFont } = await embedFonts(pdf)
  ...
  // Header
  draw(document.title, 26, {
    bold: true,
    color: rgb(0.08, 0.08, 0.08),
    leading: 32,
  })
  draw(document.subtitle, 12, {
    bold: true,
    color: rgb(0.3, 0.3, 0.3),
    leading: 17,
  })
  draw(document.contactLine, 9, {
  ...
  for (const section of document.sections) {
    ensureSpace(50)
    draw(section.title.toUpperCase(), 9.5, {
      bold: true,
      color: rgb(0.08, 0.08, 0.08),
      leading: 13,
    })
...
async function renderExecutive(document: ResumeDocument): Promise<Uint8Array> {
  ...
  const draw = (
    ...
    const font = opts.bold ? boldFont : regularFont
    const leading = opts.leading ?? size * 1.55
    const lines = wrapText(value, font, size, opts.width ?? contentWidth)
    ensureSpace(lines.length * leading)
    for (const line of lines) {
      page.drawText(line, {
        x: xPos,
        ...
        size,
        font,
        color: opts.color ?? rgb(0.13, 0.18, 0.25),
      })
      y -= leading
    }
  }
  ...
  page.drawText(document.contactLine, {
    x: margin,
    y: hBase - 50,
    size: 9,
    font: regularFont,
  ...
  for (const section of document.sections) {
    ensureSpace(52)
    draw(section.title.toUpperCase(), 10, margin, {
      bold: true,
      color: navy,
      leading: 14,
    })
...
async function renderModern(document: ResumeDocument): Promise<Uint8Array> {
  ...
  const newModernPage = () => {
    ...
    drawSidebarBg(page)
    yMain = pageSize[1] - 44
    // A sidebar não continua em páginas seguintes
  }
  ...
  const drawSide = (
    ...
    const lines = wrapText(value, font, size, sidebarContentW)
    for (const line of lines) {
      if (ySide > mainPad) {
        page.drawText(line, {
          x: sidebarPad,
  ...
  const drawMain = (
    ...
    const font = opts.bold ? boldFont : regularFont
    const leading = opts.leading ?? size * 1.55
    const lines = wrapText(value, font, size, mainWidth)
  ...
  const sidebarSections = document.sections.filter(
    (s) => s.title === 'Competencias',
  )
  ...
  for (const section of sidebarSections) {
    drawSide(section.title.toUpperCase(), 8.5, {
      bold: true,
      color: rgb(0.65, 0.85, 1),
      leading: 13,
    })
    ySide -= 2
    page.drawLine({
      start: { x: sidebarPad, y: ySide },
      end: { x: sidebarWidth - sidebarPad, y: ySide },
      thickness: 0.5,
      color: rgb(0.4, 0.65, 0.85),
    })
    ySide -= 10
    for (const item of section.items) {
      for (const skill of item.split(' | ')) {
        drawSide(`• ${skill.trim()}`, 9, {
  ...
  for (const section of mainSections) {
    drawMain(section.title.toUpperCase(), 10, {
      bold: true,
      color: accentText,
      leading: 14,
    })
    yMain -= 3
    page.drawLine({
      start: { x: mainX, y: yMain },
      end: { x: pageSize[0] - mainPad, y: yMain },
      thickness: 0.75,
      color: accentText,
    })
    yMain -= 12
...
async function renderFirstJob(document: ResumeDocument): Promise<Uint8Array> {
  ...
  for (const section of document.sections) {
    ...
    draw(section.title.toUpperCase(), isEducation ? 11.5 : 10, {
      bold: true,
      color: green,
      leading: 15,
    })
...
async function renderCreative(document: ResumeDocument): Promise<Uint8Array> {
  ...
  for (const section of document.sections) {
    ...
    page.drawRectangle({
      ...
      height: 16,
      color: rgb(0.96, 0.91, 0.98),
    })
    draw(section.title.toUpperCase(), 10, {
      bold: true,
      color: purple,
      leading: 14,
    })
...
export const renderResumePdf: ResumePdfRenderer = async (
  document,
